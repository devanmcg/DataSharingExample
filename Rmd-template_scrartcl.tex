\documentclass[$if(fontsize)$$fontsize$,$endif$$if(lang)$$babel-lang$,$endif$$if(papersize)$$papersize$,$endif$$for(classoption)$$classoption$$sep$,$endfor$]{scrartcl}
...
$if(title)$
\title{$title$$if(subtitle)$ \newline \\ \Large \emph{$subtitle$} $endif$ $if(anonymous)$$else$$if(thanks)$\thanks{$thanks$} $endif$$endif$ }
$endif$ 
...
$if(anonymous)$\hfill $else$$for(author)$\textbf{\authorfont $author.name$} \newline \hspace{0.1in} \vspace{0.05in}  \emph{\small $author.affiliation$}  $sep$ \par $endfor$ $endif$
